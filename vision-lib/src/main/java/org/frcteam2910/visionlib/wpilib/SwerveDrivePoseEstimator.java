...
package org.frcteam2910.visionlib.wpilib;
...
/**
 * This class wraps an {@link UnscentedKalmanFilter Unscented Kalman Filter} to
 * fuse latency-compensated vision measurements with swerve drive encoder
 * velocity measurements. It will correct for noisy measurements and encoder
 * drift. It is intended to be an easy but more accurate drop-in for
 * {@link edu.wpi.first.math.kinematics.SwerveDriveOdometry}.
 *
 * <p>
 * {@link SwerveDrivePoseEstimator#update} should be called every robot loop. If
 * your loops are faster or slower than the default of 0.02s, then you should
 * change the nominal delta time using the secondary constructor:
 * {@link SwerveDrivePoseEstimator#SwerveDrivePoseEstimator(Rotation2d, Pose2d, SwerveDriveKinematics, Matrix, Matrix, Matrix, double)}.
 *
 * <p>
 ...
 * and angular rate in the field coordinate system.
 *
 * <p>
...
public class SwerveDrivePoseEstimator {
     ...
     * @param stateStdDevs
     *            Standard deviations of model states. Increase these numbers to
     *            trust your model's state estimates less. This matrix is in the
     *            form [x, y, theta]ᵀ, with units in meters and radians.
     * @param localMeasurementStdDevs
     *            Standard deviation of the gyro measurement. Increase this number
     *            to trust sensor readings from the gyro less. This matrix is in the
     *            form [theta], with units in radians.
     * @param visionMeasurementStdDevs
     *            Standard deviations of the vision measurements. Increase these
     *            numbers to trust global measurements from vision less. This matrix
     *            is in the form [x, y, theta]ᵀ, with units in meters and radians.
     */
    public SwerveDrivePoseEstimator(Rotation2d gyroAngle, Pose2d initialPoseMeters, SwerveDriveKinematics kinematics,
        ...
        this(gyroAngle, initialPoseMeters, kinematics, stateStdDevs, localMeasurementStdDevs, visionMeasurementStdDevs,
                0.02);
    }
     ...
     *            form [x, y, theta]ᵀ, with units in meters and radians.
     * @param localMeasurementStdDevs
     *            Standard deviations of the encoder and gyro measurements. Increase
     *            these numbers to trust sensor readings from encoders and gyros
     *            less. This matrix is in the form [theta], with units in radians.
     * @param visionMeasurementStdDevs
     *            Standard deviations of the vision measurements. Increase these
     *            numbers to trust global measurements from vision less. This matrix
     *            is in the form [x, y, theta]ᵀ, with units in meters and radians.
     * @param nominalDtSeconds
    ...
    public SwerveDrivePoseEstimator(Rotation2d gyroAngle, Pose2d initialPoseMeters, SwerveDriveKinematics kinematics,
        ...
        m_observer = new UnscentedKalmanFilter<>(Nat.N3(), Nat.N1(), (x, u) -> u, (x, u) -> x.extractRowVector(2),
                stateStdDevs, localMeasurementStdDevs, AngleStatistics.angleMean(2), AngleStatistics.angleMean(0),
                AngleStatistics.angleResidual(2), AngleStatistics.angleResidual(0), AngleStatistics.angleAdd(2),
                m_nominalDt);
        ...
        m_visionCorrect = (u, y) -> m_observer.correct(Nat.N3(), u, y, (x, u1) -> x, m_visionContR,
                AngleStatistics.angleMean(2), AngleStatistics.angleResidual(2), AngleStatistics.angleResidual(2),
     ...
     *            {@link SwerveDrivePoseEstimator#updateWithTime} then you must use
     *            a timestamp with an epoch since FPGA startup (i.e. the epoch of
     *            this timestamp is the same epoch as Timer.getFPGATimestamp.) This
     *            means that you should use Timer.getFPGATimestamp as your time
     *            source or sync the epochs.
     */
    public void addVisionMeasurement(Pose2d visionRobotPoseMeters, double timestampSeconds) {
        var sample = m_poseBuffer.getSample(timestampSeconds);
        if (sample.isPresent()) {
            m_visionCorrect.accept(new MatBuilder<>(Nat.N3(), Nat.N1()).fill(0.0, 0.0, 0.0), StateSpaceUtil
                    .poseTo3dVector(getEstimatedPosition().transformBy(visionRobotPoseMeters.minus(sample.get()))));
        }
    }
     ...
     *            The current gyro angle.
     * @param moduleStates
     *            The current velocities and rotations of the swerve modules.
    ...
    public Pose2d updateWithTime(double currentTimeSeconds, Rotation2d gyroAngle, SwerveModuleState... moduleStates) {
        ...
        var fieldRelativeVelocities = new Translation2d(chassisSpeeds.vxMetersPerSecond,
                chassisSpeeds.vyMetersPerSecond).rotateBy(angle);
